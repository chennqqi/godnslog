...
分析：
- 部署模式：当前仅有单一部署方式，用户需要两种模式以适配不同环境
  - Let's Encrypt独立托管模式适合独立服务器/容器直接对外，需要自动证书管理
  - Nginx模式适合已有反向代理基础设施的环境，后端不需要处理TLS和特权端口
- Demo模式：方便用户快速体验产品，需要预置数据和权限限制
  - Demo账号必须权限受限，避免演示环境被滥用
