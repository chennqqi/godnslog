...
- **双部署模式支持**（每种模式均支持容器部署和非容器部署）：
  - ✅ **Let's Encrypt 自动证书独立托管模式**：
    - 直接对外暴露，监听 443/80/53 端口
    - 自动申请并续期 Let's Encrypt 证书
    - 内置自签名证书兜底（证书申请失败时仍可启动）
    - DNSLog 验证功能在 443 和 80 上均兼容
    - Web 界面和 API 始终在 443 上提供服务
    - **容器部署**：Docker/K8s 镜像直接映射 443/80/53，容器内以非特权用户启动需配合 `NET_BIND_SERVICE` 或 `setcap`
    - **非容器部署**：二进制直接运行，通过 systemd 管理服务，端口绑定 443/80/53
  - ✅ **Nginx 反向代理模式**：
    ...
    - **容器部署**：GODNSLOG 容器监听高位端口，Nginx 可为独立容器或宿主机 Nginx，通过 Docker network / K8s Service 转发
    - **非容器部署**：GODNSLOG 二进制监听高位端口或 Unix Socket，宿主机 Nginx 直接反代
- **Demo 模式支持**：
