...
test.describe('Payload Detail', () => {
  ...
  test('should display associated case', async ({ page }) => {
    await expect(page.locator('text=Associated Case').first()).toBeVisible();
    await expect(page.locator('text=Test Case').first()).toBeVisible();
  });

  test('should display recent interactions', async ({ page }) => {
    await expect(page.locator('text=Recent Interactions').first()).toBeVisible();
  ...
  test('should display revoke button for active payload', async ({ page }) => {
    // Revoke button is shown when payload.status !== 'archived'
    const revokeButton = page.locator('button').filter({ hasText: 'Revoke' });
