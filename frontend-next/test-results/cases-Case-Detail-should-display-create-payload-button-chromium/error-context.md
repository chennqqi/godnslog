...
- Name: cases.spec.ts >> Case Detail >> should display create payload button
- Location: e2e/cases.spec.ts:158:7
...
Locator: locator('button').filter({ hasText: 'Create Payload' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: 'Create Payload' }).first()
