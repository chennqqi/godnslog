...
- Name: cases.spec.ts >> Cases Board >> should display status filter
- Location: e2e/cases.spec.ts:36:7
...
Locator: locator('button').filter({ hasText: 'All statuses' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button').filter({ hasText: 'All statuses' }).first()
